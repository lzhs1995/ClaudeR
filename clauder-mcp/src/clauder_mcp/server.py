...
def format_async_progress(progress: Any) -> str:
    """Render a progress sidecar without exposing JSON empty-object sentinels."""
    if not isinstance(progress, dict) or not progress.get("stage"):
        return ""
    progress_bits = [f"stage={progress['stage']}"]
    percent = progress.get("percent")
    if percent not in (None, "", [], {}):
        progress_bits.append(f"percent={percent}")
    if progress.get("message"):
        progress_bits.append(f"message={progress['message']}")
    if progress.get("updated_at"):
        progress_bits.append(f"updated_at={progress['updated_at']}")
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[types.TextContent | types.ImageContent]:
    ...
    """
        ...
        if status == "running":
            ...
            rendered_progress = format_async_progress(result.get("progress") or {})
            progress_text = f" Latest progress: {rendered_progress}." if rendered_progress else ""
            ...
            return [types.TextContent(
                ...
                text=(
                    ...
                    f"{progress_text}{metadata_text} "
                    f"Call get_async_result(\"{job_id}\") again to check."
                )
            )]
        ...
        if not result.get("success", False):
            rendered_progress = format_async_progress(result.get("progress") or {})
            progress_text = f" Final progress: {rendered_progress}." if rendered_progress else ""
        ...
        if status == "cancelled":
            ...
            rendered_progress = format_async_progress(result.get("progress") or {})
            if rendered_progress:
                msg += f"\nFinal progress: {rendered_progress}."
            metadata_text = format_async_metadata(result)
            if metadata_text:
                msg += "\n" + metadata_text
